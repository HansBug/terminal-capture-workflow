...
| step `command` / action `wait_for_text` | `wait_until_stable` | unset | `{"ms": N}` adds a "buffer must be quiet for N ms" wait *after* the pattern matches. ttyd polls the xterm.js buffer; VHS emits a literal `Sleep <N>ms` (degraded). Use for streaming-token output (codex / claude / ollama) and spinner-heavy commands |
